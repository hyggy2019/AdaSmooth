...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        if self.args.linear_probing:
            ...
            for _i, token_id in enumerate(reg.classes_):
                decoder.weight[token_id] = coef_torch[_i]
                if use_bias:
                    decoder.bias[token_id] = bias_torch[_i]
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if args.trainer == "zo":
                    ...
                    tr_loss_step = self.do_step(model, inputs)
                    loss_to_save.append(tr_loss_step.item())
                else:
                    if (
        ...
        np.save(f"{args.model_name.split('/')[-1]}_{args.opt_name}.npy", loss_to_save)
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            param.add_(scaling_factor * z * self.args.zo_eps)
    ...
    @torch.no_grad()
    def do_step(self, model, inputs):
        args = self.args

        if args.opt_name == "vanilla":
            return self.zo_step(model, inputs)
        elif args.opt_name == "zoar":
            return self.zoar_step(model, inputs)
        else:
            raise ValueError(f"Unknown optimizer name {args.opt_name}. Please use one of [vanilla, zoar_0, zoar].")
    ...
    def zo_step(self, model, inputs):
        ...
        self.projected_seeds = [self.zo_random_seed]
    ...
    def zoar_step(self, model, inputs):
        args = self.args
        ...
        if getattr(self, "rl_past", None) is None:
            self.rl_past = []
        
        seed = np.random.randint(1000000000)

        self.zo_perturb_parameters(random_seed=seed, scaling_factor=1)
        reward_p = self.zo_forward(model, inputs)
        self.zo_perturb_parameters(random_seed=seed, scaling_factor=-2)
        reward_m = self.zo_forward(model, inputs)
        self.zo_perturb_parameters(random_seed=seed, scaling_factor=1)
        self.rl_past.append([reward_p, seed]) # even position
        self.rl_past.append([reward_m, seed]) # odd position

        if len(self.rl_past) > (num_histories + 1) * 2:
            self.rl_past = self.rl_past[-(num_histories + 1) * 2:]

        self.projected_seeds = [p[1] for p in self.rl_past]
        rewards = [p[0] for p in self.rl_past]
        rewards = torch.tensor(rewards, device=loss.device)
        rewards = rewards - rewards.mean()
        self.projected_grad = rewards / self.args.zo_eps
        self.projected_grad[1::2].mul_(-1) # trick for reward_m where pertb should be negtive (-u_k)
    ...
    def zo_update(self, model):
        """
        Update the parameters with the estimated gradients using Adam-like optimization.
        """
        args = self.args

        if not args.opt_type == "sgd":
            if not hasattr(self, 'zo_adam_m'): # Initialize Adam parameters if not already done
                self.zo_adam_m = {}  # First moment estimates
                self.zo_adam_v = {}  # Second moment estimates
                self.zo_adam_t = 0   # Timestep counter
                for name, param in self.named_parameters_to_optim:
                    self.zo_adam_m[name] = torch.zeros_like(param)
                    self.zo_adam_v[name] = torch.zeros_like(param)
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            for seed, pgrad in zip(self.projected_seeds, self.projected_grad):
                torch.manual_seed(seed)
                z = torch.normal(mean=0, std=1, size=param.size(), device=param.device, dtype=param.dtype)
                # Compute gradient estimate
                grad += pgrad * z
            grad /= len(self.projected_seeds)
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                grad += args.weight_decay * param

            # Update biased second raw moment estimate
            if args.opt_type == "sgd":
                param.add_(- self._get_learning_rate() * grad)

            else:
                # Update biased first moment estimate
                self.zo_adam_m[name] = beta1 * self.zo_adam_m[name] + (1 - beta1) * grad

                m_hat = self.zo_adam_m[name]

                if args.opt_type == "radazo":
                    self.zo_adam_v[name] = beta2 * self.zo_adam_v[name] + \
                        (1 - beta2) * (self.zo_adam_m[name] * self.zo_adam_m[name])
                else: # normal adam
                    self.zo_adam_v[name] = beta2 * self.zo_adam_v[name] + \
                        (1 - beta2) * (grad * grad)
                
                v_hat = self.zo_adam_v[name]
                if self.zo_adam_t < 30:
                    v_hat = torch.ones_like(v_hat, device=v_hat.device)

                param.add_(- self._get_learning_rate() * m_hat / (torch.sqrt(v_hat) + eps))
