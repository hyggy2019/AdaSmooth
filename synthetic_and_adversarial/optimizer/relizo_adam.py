...
class LIZO(Optimizer):
    ...
    """
    def __init__(self, params, lr=1e-3, weight_decay=0, num_sample_per_step=8, reuse_distance_bound=0., max_reuse_rate=1., sample_norm=1e-5, orthogonal_sample=True, sample_momentum=0., fast_alg=True, line_search_fn=None, strict_lr=False, betas=(0.9, 0.99), eps=1e-8, update_rule="radazo"):
...
# ---- modified from original code ----
        state['m'] = torch.zeros(self.numel_params, device=device)
        state['v'] = torch.zeros(self.numel_params, device=device)
# ---- modified from original code ----
...
# ---- modified from original code ----
        m = state.get('m', torch.zeros(self.numel_params, device=device))
        v = state.get('v', torch.zeros(self.numel_params, device=device))
# ---- modified from original code ----
...
# ---- modified from original code ----
        ...
        num_random = self.num_sample_per_step # for fair comparison
        total_num = num_random + len(sample_idx)
# ---- modified from original code ----
        if num_random > 0:
...
# ---- modified from original code ----
        last_delta_samples = torch.zeros(total_num + 1, self.numel_params, device=device)
        sample_obj = torch.zeros(total_num + 1, device=device)
        sample_lr = torch.zeros(total_num + 1, device=device)
# ---- modified from original code ----
        if len(sample_idx) > 0:
...
# ---- modified from original code ----
        ...
        last_delta_samples = last_delta_samples[:total_num]
        sample_lr = sample_lr[:total_num]
        sample_obj = sample_obj[:total_num]
# ---- modified from original code ----
...
# ---- modified from original code ----
            if self.update_rule == "sgd":
                ...
                m.mul_(beta1).add_((1 - beta1) * last_grad)
                if self.update_rule == "radazo":
                    v.mul_(beta2).add_((1 - beta2) * (m ** 2))
                else:
                    v.mul_(beta2).add_((1 - beta2) * (last_grad ** 2))
                m_hat = m
                v_hat = v
